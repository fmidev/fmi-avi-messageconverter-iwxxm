...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:template match="//aixm:localType[@xsi:nil='true']"/>
   <xsl:template match="//aixm:controlType[@xsi:nil='true']"/>
   <xsl:template match="//aixm:upperLowerSeparation[@xsi:nil='true']"/>
   <xsl:template match="//aixm:protectedRoute[@xsi:nil='true']"/>
   ...
   <!-- Remove the directionOfMotion element if it is nil, meaning no movement.
        This is different from stationary movement in IWXXM 2023-1 -->
   <xsl:template match="//iwxxm:directionOfMotion[@xsi:nil='true']"/>

   <!-- Clear the motion value in case of stationary movement -->
   <xsl:template match="//iwxxm:directionOfMotion[@nilReason]/text()"/>
