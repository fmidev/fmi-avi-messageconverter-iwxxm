...
public abstract class SIGMETIWXXMSerializer<T> extends AbstractIWXXM2023_1Serializer<SIGMET, T> {
    ...
    private SIGMETPositionCollectionType createFPA(final List<PhenomenonGeometry> fcs,
                                                   final String fcTime, final boolean noVaExp) {
        ...
        SIGMETPositionCollectionType spc = create(SIGMETPositionCollectionType.class, spct -> {
    ...
    public ConversionResult<T> convertMessage(final SIGMET input, final ConversionHints hints) {
        ...
        if (!input.getCancelledReference().isPresent()) {
            ...
            final AnalysisAndForecastPositionAnalysisType analysis = create(AnalysisAndForecastPositionAnalysisType.class);
            SIGMETEvolvingConditionCollectionType secct =
                    createAnalysis(ans, analysisTime, startTime,
                            noForecasts);
            ...
            if (sigmetPhenomen.equals(AviationCodeListUser.AeronauticalSignificantWeatherPhenomenon.VA)
                ...
                ((VolcanicAshSIGMETType) sigmet).setEruptingVolcano(create(VolcanoPropertyType.class, vpt -> {
    ...
    private SIGMETEvolvingConditionCollectionType createAnalysis(
        ...
        SIGMETEvolvingConditionCollectionType sigmetEvolvingConditionCollectionType = create(
                            ...
                            seccpt.setSIGMETEvolvingCondition(create(SIGMETEvolvingConditionType.class, sect -> {
                                if (an.getIntensityChange().isPresent()) {
                                    switch (an.getIntensityChange().get()) {
                                        case INTENSIFYING:
                                            sect.setIntensityChange(SIGMETExpectedIntensityChangeType.INTENSIFY);
                                            break;
                                        case WEAKENING:
                                            sect.setIntensityChange(SIGMETExpectedIntensityChangeType.WEAKEN);
                                            break;
                                        case NO_CHANGE:
                                            sect.setIntensityChange(SIGMETExpectedIntensityChangeType.NO_CHANGE);
                                            break;
                                    }
                                } else {
                                    sect.setIntensityChange(SIGMETExpectedIntensityChangeType.NO_CHANGE);
